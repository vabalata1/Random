...
chat = game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest
...
function Time(targetpos)
     local tme = (Players.LocalPlayer.Character.PrimaryPart.Position - targetpos).Magnitude / 16
     return tme
 end 
...
function walkToPlayer(plr)
    for i,v in next, workspace:GetDescendants() do
       if v.IsA(v,'Seat') then v:Destroy() end
    end
   Players.LocalPlayer.Character.Humanoid:MoveTo(plr.Position)
...
Players.LocalPlayer.leaderstats.Raised.Changed:Connect(function()
    ...
    sound = Instance.new('Sound',game.CoreGui)
    sound.SoundId = 'rbxassetid://9062874339'
    sound:Play()
       ...
       for i,v in next, Players:GetPlayers() do
        task.wait()
          if v ~= Players.LocalPlayer then
            pcall(function()
             walkToPlayer(v.Character.HumanoidRootPart)
            end)
          end
       end
    end
